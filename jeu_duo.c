...
int jeu_duo (int taille) {		/*	Fonction qui intègre les règles du jeu pour une partie avec un joueur 2	*/
	...
	int grille[taille][taille] ,l, h, fin, o, j, i, i2, z,position_x, position_y, position_xtab, position_ytab, x=((taille*60)/2), y=70, l_et_h=60,flag=0, remove_x, remove_y, remove_xtab, remove_ytab, remove_x2, remove_y2, remove_xtab2, remove_ytab2,nombre_tour,adjacent,test_x,test_y,gagnant;
	couleur c_cache;
	...
	for(i=0,i2=0,fin=0,z=1,nombre_tour=0;fin<1000;z++) {	/*	1001/1002 sont les valeurs qui détermineront la victoire de tel joueur	*/
		...
		if((position_x>x && position_x<(x+(taille*l_et_h)) ) && (position_y>y && position_y<(y+(taille*l_et_h)))) { /*	Je teste si on est dans le plateau	*/
			...
			if( nombre_tour==1 && z==3) {
				...
				if ( position_ytab==0  && (position_xtab>=0  && position_xtab<=taille-1  )) {
					
					if(grille[position_xtab][position_ytab]==2) {

					grille[position_xtab][position_ytab]=1;
					remplissage_pion2(taille, position_x, position_y);
					remove_xtab2=position_xtab;
					remove_ytab2=position_ytab;
					remove_x2=(position_xtab*l_et_h)+x;
					remove_y2=(position_ytab*l_et_h)+y;
					i2++;
				...
				if(grille[position_xtab][position_ytab]==2) {
					...
					if(z==1 || z==3) {
						...
						if(z==3 && nombre_tour!=1) {
							...
							adjacent=adjacente(remove_xtab2,remove_ytab2,position_xtab,position_ytab);
						...
						if(z==4) {  /*	Croix noir	*/
						grille[position_xtab][position_ytab]=0;
						remplissage_croix2(taille, position_x, position_y);
						flag=1;
